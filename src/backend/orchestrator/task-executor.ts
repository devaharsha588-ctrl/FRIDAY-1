...
export async function startTask(
  ...
  try {
    await executeLoop(task.id, input.goal, taskStore, onEvent, runnerOptions, () => timedOut);
  } finally {
...
async function executeLoop(
  ...
  for (const action of plannedActions) {
    ...
    // Register the action in the task
    ...
    if (result.status === 'success' || result.status === 'completed') {
    ...
    } else if (result.status === 'failed' || result.status === 'blocked' || result.status === 'unsupported') {
      ...
      // needs_confirmation returned from agent, or other status — treat as failure
      onEvent({ type: 'action_failed', taskId, result });
